...
public abstract class SelectionMenu extends DefaultMenu {
    ...
    private String generateHeader(String s){
        double firstStage = Math.ceil(((double)(BOX_LENGHT - s.length()) / 2)) - Math.ceil((double)HEADER_SPACING / 2);
        double secondStage = Math.floor(((double)(BOX_LENGHT - s.length()) / 2)) - Math.floor((double)HEADER_SPACING / 2);
        double firstSpacing = Math.ceil((double)HEADER_SPACING / 2);
        double secondSpacing = Math.floor((double)HEADER_SPACING / 2);
